...
(module meat GOVERNANCE
  ...
  (defun leaderboard ()
    (take 10 (sort [ 'balance ] (list-accounts)))
  )
  ...
  (defun rotate:string (account:string new-guard:guard)
    (with-read ledger account { "guard":= old-guard }
  ...
  (defun credit:string (account:string guard:guard amount:decimal)
    ...
    (with-default-read ledger account
      ...
      (write ledger account
        { "balance": (+ balance amount), "guard": retg}
      )
    )
  )
...
(if (read-msg "upgrade") ["Module upgraded"] [(create-table ledger)])
