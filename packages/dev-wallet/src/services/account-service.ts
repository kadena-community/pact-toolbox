...
import type { PactToolboxContext, ChainId } from '@pact-toolbox/types';
...
export class AccountService {
  ...
  private networkContext?: PactToolboxContext;

  constructor(storage?: DevWalletStorage, networkContext?: PactToolboxContext) {
    this.storage = storage || new DevWalletStorage();
    this.networkContext = networkContext;
    
    // Initialize CoinService if network context is provided
    if (networkContext) {
  ...
  async saveAccount(account: Account): Promise<void> {
    try {
      ...
      if (this.networkContext && account.privateKey) {
        try {
  ...
  async createAccountOnBlockchain(account: DevWalletKey, chainId: ChainId = '0'): Promise<boolean> {
    try {
      if (!this.coinService || !this.networkContext) {
        accountLogger.warn('CoinService or NetworkContext not initialized');
        return false;
      }
   ...
   */
  setNetworkContext(context: PactToolboxContext): void {
