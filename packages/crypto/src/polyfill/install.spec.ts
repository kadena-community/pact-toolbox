...
describe("exportKey() polyfill", () => {
  ...
  describe("when installed", () => {
    ...
    it("delegates `exportKey` calls to the polyfill when supplied a polyfill-generated key", async () => {
      expect.assertions(1);
      (isPolyfilledKey as Mock).mockReturnValue(true);
    ...
    it("delegates `exportKey` calls to the original implementation when supplied a native-generated key", async () => {
      expect.assertions(1);
      (isPolyfilledKey as Mock).mockReturnValue(false);
      const mockPublicKey = {} as CryptoKey;
      try {
...
describe("generateKey() polyfill", () => {
  ...
  describe("when installed in an environment with no `generateKey` function", () => {
    ...
    it.each(ED25519_ALGORITHM_IDENTIFIERS)(
      ...
      async (algorithm) => {
        expect.assertions(1);
        const mockKeyPair = {};
        (generateKeyPolyfill as Mock).mockReturnValue(mockKeyPair);
        await expect(
          globalThis.crypto.subtle.generateKey(algorithm, /* extractable */ false, ["sign", "verify"]),
  ...
  describe("when installed in an environment that does not support Ed25519", () => {
    beforeEach(() => {
      ...
      (originalGenerateKey as Mock<typeof originalGenerateKey>).mockImplementation(async (...args) => {
  ...
  describe("when installed in an environment that supports Ed25519", () => {
    ...
    it.each(ED25519_ALGORITHM_IDENTIFIERS)(
      ...
      async (algorithm) => {
        expect.assertions(1);
        const mockKeyPair = {};
        (originalGenerateKey as Mock).mockResolvedValue(mockKeyPair);
        await expect(
          globalThis.crypto.subtle.generateKey(algorithm, /* extractable */ false, ["sign", "verify"]),
        ).resolves.toBe(mockKeyPair);
      },
  ...
  describe("when installed with no `sign` function", () => {
    ...
    it.each(ED25519_ALGORITHM_IDENTIFIERS)(
      ...
      async (algorithm) => {
        ...
        const mockPrivateKey = {} as CryptoKey;
        const mockData = new Uint8Array([1, 2, 3]);
        await expect(() => globalThis.crypto.subtle.sign(algorithm, mockPrivateKey, mockData)).rejects.toThrow();
      },
    );
  });
  describe("when installed in an insecure context", () => {
...
describe("importKey() polyfill", () => {
  ...
  beforeEach(() => {
    vi.spyOn(globalThis.crypto?.subtle, "importKey");
  ...
  describe("when imported in an environment with no `importKey` function", () => {
    ...
    it.each(ED25519_ALGORITHM_IDENTIFIERS)(
      ...
      async (algorithm) => {
        expect.assertions(1);
        const mockKey = {};
        (importKeyPolyfill as Mock).mockReturnValue(mockKey);
        await expect(
          globalThis.crypto.subtle.importKey("raw", MOCK_PUBLIC_KEY_BYTES, algorithm, /* extractable */ false, [
            "verify",
          ]),
  ...
  describe("when imported in an environment that does not support Ed25519", () => {
    beforeEach(() => {
      ...
      (originalImportKey as unknown as Mock<typeof originalImportKey>).mockImplementation(async (...args) => {
  ...
  describe("when imported in an environment that supports Ed25519", () => {
    ...
    it.each([
      ...
      expect.assertions(1);
      try {
        await globalThis.crypto.subtle.importKey("raw", MOCK_PUBLIC_KEY_BYTES, algorithm, /* extractable */ false, [
          "verify",
        ]);
      } catch {
    ...
    it.each(ED25519_ALGORITHM_IDENTIFIERS)(
      ...
      async (algorithm) => {
        expect.assertions(1);
        const mockKey = {};
        (originalImportKey as Mock).mockResolvedValue(mockKey);
        await expect(
          globalThis.crypto.subtle.importKey("raw", MOCK_PUBLIC_KEY_BYTES, algorithm, /* extractable */ false, [
            "verify",
          ]),
