...
const MOCK_DATA_SIGNATURE = new Uint8Array([
  ...
  245, 181, 165, 19, 73, 242, 201, 84, 233, 251, 88, 27, 10, 18, 59, 1, 101, 228, 9, 14, 200, 66, 233, 195, 13, 79, 72,
  48, 6, 161, 22, 137, 127, 73, 135, 139, 150, 125, 11,
]);
const MOCK_SECRET_KEY_BYTES = new Uint8Array([
  83, 147, 250, 112, 140, 37, 29, 73, 156, 38, 185, 76, 163, 8, 178, 225, 172, 53, 120, 108, 127, 191, 103, 8, 160, 170,
  183, 186, 246, 1, 227, 158,
]);
const MOCK_SECRET_KEY_BASE_64_URL = "U5P6cIwlHUmcJrlMowiy4aw1eGx_v2cIoKq3uvYB454";
const MOCK_PUBLIC_KEY_BYTES = new Uint8Array([
  166, 132, 114, 186, 49, 163, 23, 12, 11, 14, 119, 219, 102, 96, 26, 226, 91, 97, 238, 217, 236, 84, 232, 204, 62, 212,
  179, 252, 20, 37, 179, 52,
]);
...
describe("exportKeyPolyfill", () => {
  ...
  describe("when format is `raw`", () => {
    ...
    it.each(["private", "secret"] as KeyType[])("throws when a %s key is supplied", async (type) => {
      ...
      await expect(exportKeyPolyfill("raw", mockKey)).rejects.toThrow(`Unable to export a raw Ed25519 ${type} key`);
    });
    it("throws when supplied a public key that was not generated with the polyfill", async () => {
      expect.assertions(1);
      const { publicKey } = await crypto.subtle.generateKey("Ed25519", /* extractable */ false, ["sign", "verify"]);
    ...
    it("returns the public key bytes associated with a secret key generated by the polyfill", async () => {
      expect.assertions(1);
      vi.spyOn(globalThis.crypto, "getRandomValues").mockReturnValue(MOCK_SECRET_KEY_BYTES);
  ...
  describe("when format is `pkcs8`", () => {
    ...
    it.each(["public", "secret"] as KeyType[])("throws when a %s key is supplied", async (type) => {
      expect.assertions(1);
      const mockKey = { extractable: true, type } as unknown as CryptoKey;
      await expect(exportKeyPolyfill("pkcs8", mockKey)).rejects.toThrow(`Unable to export a pkcs8 Ed25519 ${type} key`);
    });
    it("throws when supplied a private key that was not generated with the polyfill", async () => {
      expect.assertions(1);
      const { privateKey } = await crypto.subtle.generateKey("Ed25519", /* extractable */ false, ["sign", "verify"]);
...
describe("importKeyPolyfill", () => {
  describe("when format is `raw`", () => {
    ...
    it("stores public key bytes in an internal cache", () => {
      const weakMapSetSpy = vi.spyOn(WeakMap.prototype, "set");
  ...
  describe("when format is `pkcs8`", () => {
    ...
    it("can import a private key that was exported from a native generated key", async () => {
      expect.assertions(1);
      const { privateKey } = await crypto.subtle.generateKey("Ed25519", /* extractable */ true, ["verify", "sign"]);
  ...
  describe("when format is `jwk`", () => {
    ...
    it("can import a public key that was exported from a native generated key", async () => {
      expect.assertions(1);
      const { publicKey } = await crypto.subtle.generateKey("Ed25519", /* extractable */ false, ["verify", "sign"]);
...
describe("generateKeyPolyfill", () => {
  it("stores secret key bytes in an internal cache", () => {
    ...
    vi.spyOn(globalThis.crypto, "getRandomValues").mockReturnValue(expectedSecretKey);
...
describe("verifyPolyfill", () => {
  ...
  it('throws when the key supplied has no "verify" usage', async () => {
    ...
    await expect(verifyPolyfill(mockKey, mockSignature, MOCK_DATA)).rejects.toThrow(/Unable to use this key to verify/);
  });
  it.each(["private", "secret"] as KeyType[])("throws when a %s key is supplied", async (type) => {
    expect.assertions(1);
