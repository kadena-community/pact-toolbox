...
describe("@pact-toolbox/crypto", () => {
  ...
  describe("Key Generation", () => {
    ...
    test("genKeyPair generates and exports keypair", async () => {
      ...
      mockCrypto.subtle.exportKey.mockResolvedValueOnce(pubKeyBytes).mockResolvedValueOnce(privKeyBytesPkcs8);
    ...
    test("createKeyPairFromBytes creates keypair from 64 bytes", async () => {
      ...
      mockCrypto.subtle.importKey.mockResolvedValueOnce(mockPublicKey).mockResolvedValueOnce(mockPrivateKey);
  ...
  describe("Signing and Verification", () => {
    ...
    test("signBytes signs message", async () => {
      ...
      expect(mockCrypto.subtle.sign).toHaveBeenCalledWith("Ed25519", mockPrivateKey, message);
    ...
    test("signBytes signs UTF-8 encoded string", async () => {
      ...
      expect(mockCrypto.subtle.sign).toHaveBeenCalledWith("Ed25519", mockPrivateKey, messageBytes);
      expect(sig).toEqual(signature);
    });
    ...
    test("verifySignature verifies valid signature", async () => {
      ...
      const isValid = await verifySignature(mockPublicKey as any, signature as SignatureBytes, message);

      expect(mockCrypto.subtle.verify).toHaveBeenCalledWith("Ed25519", mockPublicKey, signature, message);
