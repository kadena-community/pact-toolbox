...
import { resolve, optional } from "@pact-toolbox/utils";
import { TOKENS } from "@pact-toolbox/types";

/**
 * Configuration for the CoinService with DI
 */
export interface CoinServiceConfigDI {
...
export class CoinServiceDI {
  ...
  constructor(config?: CoinServiceConfigDI) {
  ...
  async transfer(options: TransferOptions): Promise<string> {
    ...
    return execution(`(coin.transfer "${from}" "${to}" ${amount})`)
      ...
      .withSigner(signerKeys[0], (withCapability) => [
  ...
  /**
   * Get coin supply on a specific chain
   */
  async getTotalSupply(options?: CoinOperationOptions): Promise<string> {
    const chainId = options?.chainId || this.defaultChainId;
    const networkConfig = this.getNetworkConfig();

    const result = await execution<any>(`(coin.total-supply)`)
      .withChainId(chainId)
      .withNetwork(networkConfig)
      .build()
      .dirtyRead();
 ...
 */
export function createCoinService(config?: CoinServiceConfigDI): CoinServiceDI {
  return new CoinServiceDI(config);
}