 ...
 */
export interface MarmaladeServiceConfigDI {
 ...
 */
export class MarmaladeServiceDI {
  ...
  constructor(config?: MarmaladeServiceConfigDI) {
  ...
  async createToken(options: CreateTokenOptions): Promise<string> {
    ...
    return execution(`(marmalade.ledger.create-token 
      "${id}" 
      ${precision} 
      "${uri}" 
      [${policyList}] 
      ...
      .withSigner(creatorKey, (withCapability) => [
        withCapability("marmalade.ledger.TOKEN", id),
      ])
  ...
  async mint(options: MintTokenOptions): Promise<string> {
    ...
    return execution(`(marmalade.ledger.mint 
      "${tokenId}" 
      "${account}" 
  ...
  async transferCreate(options: TransferCreateTokenOptions): Promise<string> {
    ...
    return execution(`(marmalade.ledger.transfer-create 
      "${tokenId}" 
      "${from}" 
      "${to}" 
      (read-keyset 'to-guard) 
      ${amount})`)
      .withChainId(resolvedChainId)
      .withNetwork(networkConfig)
      .withMeta({
 ...
 */
export function createMarmaladeService(config?: MarmaladeServiceConfigDI): MarmaladeServiceDI {
  return new MarmaladeServiceDI(config);
}