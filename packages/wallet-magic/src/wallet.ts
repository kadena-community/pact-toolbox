import type { PartiallySignedTransaction, SignedTransaction, WalletEvents } from "@pact-toolbox/types";
import type { WalletAccount } from "@pact-toolbox/wallet-core";
import { WalletError } from "@pact-toolbox/wallet-core";
import type { MagicOptions, MagicSDK, MagicSpireKeyAccount, MagicTransaction, MagicEvents } from "./types";
...
export class MagicWallet extends BaseWallet {
   ...
   * Check if connected
   */
  override async isConnected(_networkId?: string): Promise<boolean> {
    if (!this.magic) {
      return false;
    }
   ...
   */
  override async disconnect(_networkId?: string): Promise<void> {
    if (this.magic) {
      try {
