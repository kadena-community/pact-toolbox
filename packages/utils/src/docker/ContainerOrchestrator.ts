...
export class ContainerOrchestrator {
 ...
 #logger = logger.withTag("ContainerOrchestrator");
  ...
  async #getOrCreateNetwork(): Promise<void> {
    try {
      ...
      if (error.statusCode === 404) {
        ...
        this.#logger.error(
          `Error inspecting/creating network ${this.#networkName}:`,
          error.message || error
        );
        throw error;
      }
    ...
    if (!this.#networkId) {
      throw new Error(
        `Failed to obtain network ID for '${this.#networkName}'`
      );
    }
  }
  ...
  #resolveServiceOrder(services: DockerServiceConfig[]): string[] {
    ...
    const visit = (serviceGroupName: string): void => {
      ...
      if (visiting.has(serviceGroupName)) {
        throw new Error(
          `Circular dependency detected: ${serviceGroupName}`
        );
      }
  ...
  async startServices(serviceConfigs: DockerServiceConfig[]): Promise<void> {
    ...
    this.#logger.info(
      `Service group startup order: ${orderedServiceGroupNames.join(
        ", "
      )}`
    );
    ...
    for (const serviceGroupName of orderedServiceGroupNames) {
      const config = serviceConfigs.find(
        (s) => s.containerName === serviceGroupName
      )!;
      if (!config) {
        this.#logger.warn(
          `Config for service group '${serviceGroupName}' not found. Skipping.`
        );
        continue;
      }
      ...
      this.#logger.info(
        `Preparing to start ${replicaCount} instance(s) of service group '${serviceGroupName}'...`
      );

      for (let i = 0; i < replicaCount; i++) {
        const instanceName =
          replicaCount > 1 ? `${serviceGroupName}-${i + 1}` : serviceGroupName;
        ...
        if (config.dependsOn) {
          for (const depGroupName of Object.keys(config.dependsOn)) {
            const depServiceGroupInstances =
              this.#runningServices.get(depGroupName);
            if (
              !depServiceGroupInstances ||
              depServiceGroupInstances.length === 0
            ) {
              throw new Error(
            ...
            if (
              config.dependsOn[depGroupName]?.condition === "service_healthy"
            ) {
              this.#logger.start(
              ...
              try {
                await Promise.all(
                  depServiceGroupInstances.map((depInstance) =>
                    depInstance.waitForHealthy()
                  )
                );
                this.#logger.success(
                  `All instances of '${depGroupName}' are healthy for '${instanceName}'.`
                );
                ...
                throw new Error(
                  `Dependency group '${depGroupName}' for '${instanceName}' failed to become healthy.`
                );
              }
            }
          }
    ...
    this.#logger.success(
      `All provided service groups attempted to start.`
    );
  }
    ...
    for (const serviceInstances of this.#runningServices.values()) {
      for (const service of serviceInstances) {
        service.streamLogs().catch((err) => {
          this.#logger.error(
            `Error starting log stream for ${service.serviceName}: ${err.message}`
          );
        });
      }
    }
  ...
  async stopAllServices(): Promise<void> {
    ...
    const serviceGroupNamesToStop = Array.from(
      this.#runningServices.keys()
    ).reverse();
    ...
    for (const serviceGroupName of serviceGroupNamesToStop) {
      ...
      if (serviceInstances) {
        ...
        this.#logger.success(
          `All instances of service group '${serviceGroupName}' stopped and removed.`
        );
      }
    }
    ...
    if (this.#networkId) {
      try {
        ...
        if (
          netInfo &&
          netInfo.Containers &&
          Object.keys(netInfo.Containers).length > 0
        ) {
          this.#logger.warn(
            `Network '${this.#networkName}' (ID: ${
              this.#networkId
            }) still has containers: ${Object.keys(netInfo.Containers).join(
              ", "
            )}. Manual cleanup may be required.`
          );
        } else if (netInfo) {
          this.#logger.info(
            `Removing network '${this.#networkName}' (ID: ${this.#networkId})...`
          );
          await network.remove();
          this.#logger.success(
            `Network '${this.#networkName}' removed.`
          );
        } else {
          this.#logger.info(
        ...
        if (error.statusCode === 404) {
          this.#logger.info(
            `Network '${this.#networkName}' (ID: ${this.#networkId}) was already removed.`
          );
        } else {
          this.#logger.warn(
            `Error removing network '${this.#networkName}':`,
            error.message || error
          );
        }
      }
