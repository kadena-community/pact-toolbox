...
import type { WalletLike, WalletManagerLike } from "@pact-toolbox/types";
...
/**
 * Type guard to check if an object implements IWalletManager
 */
export function isWalletManagerLike(obj: unknown): obj is WalletManagerLike {
  return (
    typeof obj === "object" &&
    obj !== null &&
...
export function getGlobalWalletManager(): WalletManagerLike | null {
  if (typeof globalThis !== "undefined" && (globalThis as any).__PACT_WALLET_MANAGER__) {
    ...
    if (isWalletManagerLike(manager)) {
      console.log("Found global wallet manager:", manager);
      return manager;
    }
  }
  return null;
...
export async function getWallet(walletLike?: WalletLike): Promise<WalletLike> {
  console.log("Attempting to retrieve wallet...", walletLike);
  if (isWalletLike(walletLike)) {
    console.log("Found wallet from walletLike:", walletLike);
    return walletLike;
  }

  // Check for global wallet manager (set by wallet-adapters)
  const manager = getGlobalWalletManager();
  if (manager) {
    // Try to get existing primary wallet
    const primaryWallet = manager.getPrimaryWallet();
    if (primaryWallet) {
      console.log("Found primary wallet from global manager:", primaryWallet);
      return primaryWallet;
    }

    // Try to auto-connect to the first available wallet
    try {
      const wallet = await manager.connect();
      console.log("Found wallet from global manager:", wallet);
      if (wallet) {
        return wallet;
      }
    } catch (error: any) {
      throw new Error(`No wallet connected and auto-connect failed: ${error?.message || "Unknown error"}`);
    }
  }

  // Fallback error for environments without wallet-manager
  throw new Error(
    "No wallet available. In browser environments, ensure @pact-toolbox/wallet-manager is initialized before using wallet features.",
  );
}
