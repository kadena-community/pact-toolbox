...
import { existsSync, copyFile, ensureDir, removeDir, logger, writeFile, calculateFileHash } from "@pact-toolbox/node-utils";
...
export async function downloadPactDependency(
  ...
  if (dep.requires) {
    await Promise.all(
      dep.requires.map((dep) => downloadPactDependency(dep, preludeDir, validateChecksum))
    );
  }
}
...
export async function downloadAllPreludes(
  ...
  if (initialStats.totalEntries > 0) {
    logger.info(`Cache contains ${initialStats.totalEntries} preludes (≈${Math.round(initialStats.totalSize / 1024)}KB)`);
  }
  ...
  let skippedCount = 0;
  for (const prelude of preludes) {
    const wasAlreadyDownloaded = downloaded.has(prelude.name) || 
      (!forceDownload && await isPreludeCached(prelude.name, extractPreludeVersion(prelude), preludesDir, !validateChecksums));
...
export async function shouldDownloadPreludes(
  ...
  const downloadStatus = await Promise.all(
    preludes.map(p => isPreludeDownloaded(p, preludesDir, validateChecksums))
  );
