 ...
 */
export async function shouldDeployPrelude(
  definition: PreludeDefinition,
  client: PactToolboxClient
): Promise<boolean> {
 ...
 */
export async function generatePreludeRepl(
  definition: PreludeDefinition,
  client: PactToolboxClient
): Promise<string> {
  ...
  if (conditions.requireMissingContracts) {
    try {
      const contractChecks = await Promise.all(
        conditions.requireMissingContracts.map(contract => 
          client.isContractDeployed(contract)
        )
      );
  ...
  if (conditions.requireMissingNamespaces) {
    try {
      const namespaceChecks = await Promise.all(
        conditions.requireMissingNamespaces.map(ns => 
          client.isNamespaceDefined(ns)
        )
      );
 ...
 */
async function generateReplScript(
  definition: PreludeDefinition,
  client: PactToolboxClient
): Promise<string> {
