...
import { generateModule, generateGasStation, generateFungible, fillTemplatePlaceholders } from "./index";
import type { ModuleContext, GasStationContext, FungibleContext } from "./index";
  ...
  describe("fillTemplatePlaceholders", () => {
    ...
    test("throws on missing placeholder", () => {
      ...
      expect(() => fillTemplatePlaceholders(template, context)).toThrow(
        "Missing required context values for keys: age",
      );
    });
...
(module {{name}} {{adminKeyset}}
  ...
  describe("Error Cases", () => {
    ...
    test("fillTemplatePlaceholders handles nested placeholders", () => {
      ...
      expect(() => fillTemplatePlaceholders(template, context)).toThrow(
        "Missing required context values for keys: outer{{inner",
      );
    });
  });
});
