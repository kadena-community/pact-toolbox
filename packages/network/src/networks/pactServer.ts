 ...
 */
function configToYaml(config: PactServerConfig, version: string): string {
  let yaml = "# Generated by @pact-toolbox/network\n";

  if (config.persistDir) {
    config.persistDir = config.persistDir.replace("{version}", version);
  }
  if (config.logDir) {
    config.logDir = config.logDir.replace("{version}", version);
  }
...
export class PactServerNetwork implements NetworkApi {
  ...
  constructor(networkConfig: PactServerNetworkConfig, deployer: PactDeployer, logger: Logger = defaultLogger) {
  ...
  async start(options?: NetworkStartOptions): Promise<void> {
    ...
    // Write config file
    const version = (await getCurrentPactVersion()) ?? "unknown";

    const toolboxDir = join(process.cwd(), ".pact-toolbox/pact", version);
    const configName = stateless ? `pact-config-${this.id}.yaml` : "pact-config.yaml";
    this.configPath = join(toolboxDir, configName);
    await writeFile(this.configPath, configToYaml(this.config, version));
