...
export async function listen<T>(
  ...
  return descriptors.length === 1 ? getTxDataOrFail<T>(results[0]!) : (results.map((r) => getTxDataOrFail<T>(r)) as T[]);
}
...
export function createRpcUrlResolver(rpcUrlTemplate: string) {
  return (networkId: string, chainId: string) =>
    rpcUrlTemplate.replace("{networkId}", networkId).replace("{chainId}", chainId);
}
