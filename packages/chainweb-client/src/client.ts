...
import { createRpcUrlResolver } from "./helpers";
...
export class ChainwebClient {
  ...
  private config: ResolvedNetworkConfig;

  constructor(config: NetworkConfig) {
    const rpcUrl = typeof config.rpcUrl === "function" ? config.rpcUrl : createRpcUrlResolver(config.rpcUrl);
    this.config = {
      timeout: 30000,
      headers: {},
      ...config,
   ...
   */
  async listen(requestKey: string, config?: RequestConfig): Promise<TransactionResult> {
    if (!requestKey) {
  ...
  private async waitForResult(
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        try {
          return this.listen(requestKey, config);
        } catch {
