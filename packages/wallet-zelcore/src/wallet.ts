...
export class ZelcoreWallet extends BaseWallet {
  ...
  async connect(networkId?: string): Promise<WalletAccount> {
    try {
      ...
      if (error instanceof Error && error.message.includes("Failed to fetch")) {
        throw WalletError.connectionFailed("Cannot connect to Zelcore. Make sure Zelcore is running.");
      }
  ...
  async sign(
    ...
    const signedTxs = await Promise.all(transactions.map((tx) => this._signSingle(tx)));
