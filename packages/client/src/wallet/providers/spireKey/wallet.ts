...
export class KadenaSpireKey {
  ...
  private _transactions: { [key: string]: IUnsignedCommand } = {};
  ...
  update(location: Location) {
    ...
    if (searchParams.has("user")) {
      ...
      if (userSearch && 0 < userSearch?.length) {
    ...
    if (searchParams.has("transaction")) {
      ...
      if (transactionSearch && 0 < transactionSearch?.length) {
  ...
  private _loadFromLocalStorage() {
    ...
    if (transactions) {
      const parsedTransactions = tryParse<{ [key: string]: IUnsignedCommand }>(transactions);
