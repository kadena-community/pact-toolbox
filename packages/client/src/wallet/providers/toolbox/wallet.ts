...
export class ToolboxWallet implements Wallet {
  ...
  constructor(context: ToolboxNetworkContext) {
    ...
    if ((networkConfig as any).type === "chainweb" && networkConfig.keyPairs.length <= 0) {
  ...
  async quickSign(
    ...
    return signed.length === 1 ? finalizeTransaction(signed[0]!) : signed.map(finalizeTransaction);
  }
