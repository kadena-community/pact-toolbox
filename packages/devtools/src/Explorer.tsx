...
import { createMemo, createSignal, Index, JSX, Match, Show, Switch } from "solid-js";
import { serialize, stringify } from "superjson";

import { useQueryDevtoolsContext, useTheme } from "./contexts";
import { Check, CopiedCopier, Copier, ErrorCopier, List, Pencil, Trash } from "./icons";
import { tokens } from "./theme";
import { deleteNestedDataByPath, displayValue, updateNestedDataByPath } from "./utils";
 ...
 */
function chunkArray<T extends { label: string; value: unknown }>(array: Array<T>, size: number): Array<Array<T>> {
  if (size < 1) return [];
  let i = 0;
...
const ClearArrayButton = (props: { dataPath: Array<string>; activeQuery: Query }) => {
  const theme = useTheme();
  const css = useQueryDevtoolsContext().shadowDOMTarget
    ? goober.css.bind({ target: useQueryDevtoolsContext().shadowDOMTarget })
    : goober.css;
...
const DeleteItemButton = (props: { dataPath: Array<string>; activeQuery: Query }) => {
  const theme = useTheme();
  const css = useQueryDevtoolsContext().shadowDOMTarget
    ? goober.css.bind({ target: useQueryDevtoolsContext().shadowDOMTarget })
    : goober.css;
...
const ToggleValueButton = (props: { dataPath: Array<string>; activeQuery: Query; value: boolean }) => {
  const theme = useTheme();
  const css = useQueryDevtoolsContext().shadowDOMTarget
    ? goober.css.bind({ target: useQueryDevtoolsContext().shadowDOMTarget })
    : goober.css;
    ...
    <div class={styles().entry}>
        ...
        <div class={styles().expanderButtonContainer}>
          ...
          <Show when={props.editable}>
            <div class={styles().actions}>
              ...
              <Show when={props.itemsDeletable && props.activeQuery !== undefined}>
                <DeleteItemButton activeQuery={props.activeQuery!} dataPath={currentDataPath} />
              </Show>

              <Show when={type() === "array" && props.activeQuery !== undefined}>
                <ClearArrayButton activeQuery={props.activeQuery!} dataPath={currentDataPath} />
              </Show>
      ...
      <Show when={subEntryPages().length === 0}>
        <div class={styles().row}>
          ...
          >
            <Show
              when={props.editable && props.activeQuery !== undefined && (type() === "string" || type() === "number")}
            >
              <input
            ...
            <Show when={type() === "boolean"}>
              <span class={cx(styles().value, styles().actions, styles().editableInput)}>
                <ToggleValueButton
                  activeQuery={props.activeQuery!}
          ...
          <Show when={props.editable && props.itemsDeletable && props.activeQuery !== undefined}>
            <DeleteItemButton activeQuery={props.activeQuery!} dataPath={currentDataPath} />
          </Show>
        </div>
      </Show>
