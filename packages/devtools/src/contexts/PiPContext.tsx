...
const PiPContext = createContext<Accessor<PiPContextType> | undefined>(undefined);

export const PiPProvider = (props: PiPProviderProps): JSX.Element => {
  ...
  const requestPipWindow = (width: number, height: number) => {
    ...
    // @ts-expect-error - styleSheets is an iterable
    ...
    delegateEvents(
      ["focusin", "focusout", "pointermove", "keydown", "pointerdown", "pointerup", "click", "mousedown", "input"],
      pip.document,
    );
...
export const usePiPWindow = (): Accessor<PiPContextType> => {
