...
export function getQueryStatusLabel(query: Query): "fetching" | "inactive" | "paused" | "stale" | "fresh" {
...
export function getSidedProp<T extends string>(prop: T, side: DevtoolsPosition) {
  return `${prop}${side.charAt(0).toUpperCase() + side.slice(1)}` as `${T}${Capitalize<DevtoolsPosition>}`;
}
...
export function getQueryStatusColorByLabel(label: QueryStatusLabel): "green" | "yellow" | "purple" | "gray" | "blue" {
  return label === "fresh"
    ? "green"
    : label === "stale"
      ? "yellow"
 ...
 */
export const displayValue = (value: unknown, beautify: boolean = false): string => {
...
const getMutationStatusRank = (m: Mutation) =>
  m.state.isPaused ? 0 : m.state.status === "error" ? 2 : m.state.status === "pending" ? 1 : 3;

const mutationDateSort: MutationSortFn = (a, b) => (a.state.submittedAt < b.state.submittedAt ? 1 : -1);
...
export const getPreferredColorScheme = (): Accessor<"light" | "dark"> => {
...
export const setupStyleSheet = (nonce?: string, target?: ShadowRoot): void => {
  if (!nonce) return;
