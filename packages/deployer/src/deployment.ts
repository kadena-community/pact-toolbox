...
import type { TOKENS, IChainwebClient, INetworkProvider } from "@pact-toolbox/types";
import { resolve } from "pathe";
import type { PactToolboxClient } from "./runtime-new";
...
export class DeploymentHelper {
  ...
  constructor(
    ...
    this.chainwebClient = client.getChainwebClient();
    this.networkProvider = client.getNetworkProvider();
  }
  ...
  async deploy(contractName: string, options: AdvancedDeploymentOptions = {}): Promise<AdvancedDeployResult> {
    ...
    try {
      ...
      // Execute deployment
      const baseResult = await this.deployContract(contractName, options);
