 ...
 */
export function generateModuleCode(module: PactModule): CodeGenerator {
 ...
 */
export function generateFunctionCode(func: PactFunction): string {
  const paramList = func.parameters.map((p) => camelCase(p.name)).join(", ");
  return `export function ${camelCase(func.name)}(${paramList}) {
  return execution(\`(${func.path} ${func.parameters.map((p) => `\${JSON.stringify(${camelCase(p.name)})}`).join(" ")})\`);
}`;
}
...
export function generateFunctionTypes(func: PactFunction): string {
  const paramList = func.parameters
    ...
    .join(", ");
  return `${convertToJsDoc(func.doc)}export declare function ${camelCase(func.name)}(${paramList}): PactTransactionBuilder<PactExecPayload, ${pactTypeToTypescriptType(func.returnType, func.module)}>;`;
}
